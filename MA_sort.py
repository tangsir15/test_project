...
data['era_4_data'] = data.apply(lambda row: row['LastPrice'] if row['era'] >= 4 else None, axis=1)
data['era_down'] = data.apply(lambda row: row['LastPrice'] if row['era'] <= -4 else None, axis=1)
plt.plot(data['index'], data['LastPrice'], label='LastPrice', color='grey', alpha=0.3)
